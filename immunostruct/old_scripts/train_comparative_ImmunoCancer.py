...
from data_loading import ImmunoPredDatasetComparative, collate, collate_amino_acid, SplitDataset, ExtendedDataset, ClinicalDataset
from models.mapping import model_map
from utils import Losses, seed_everything, LinearWarmupCosineAnnealingLR, update_paths
from procedures import inference_comparative, inference_comparative_SSL, train_model_comparative, train_model_comparative_SSL
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-save-dir", default="$ROOT/checkpoints/comparative_ImmunoCancer/", type=str)
    parser.add_argument("--figure-save-dir", default="$ROOT/figures/comparative_ImmunoCancer/", type=str)
    ...
    model_str = f"{config.model}-wtds_{config.use_wt_for_downstream}" + \
        f"-lr_ft_{config.learning_rate_finetune}" + \
    ...
    wandb.init(
        ...
        name=f"Comparative-ImmunoCancer:{model_str}",  # display name of the run
