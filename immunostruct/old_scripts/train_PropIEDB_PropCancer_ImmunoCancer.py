...
if __name__ == "__main__":
    ...
    wandb.init(
        ...
        name=f"PropIEDB_PropCancer_ImmunoCancer:{model_str}",  # display name of the run
    ...
    dataset_pt2 = dataset_ft = ImmunoPredDataset(config,
    ...
    # `binary=False` --> Using property.
    train_split_dataset = SplitDataset(train_dataset_pt2, "train", binary=False, full=config.full_sequence, comparative=False, return_amino_acid=config.self_supervision)
    val_split_dataset = SplitDataset(val_dataset_pt2, "val", binary=False, full=config.full_sequence, comparative=False, return_amino_acid=config.self_supervision)
    test_split_dataset = SplitDataset(test_dataset_pt2, "test", binary=False, full=config.full_sequence, comparative=False, return_amino_acid=config.self_supervision)
