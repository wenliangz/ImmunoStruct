...
from data_loading import ImmunoPredDataset, ImmunoPredDatasetComparative, collate, collate_amino_acid, SplitDataset, ExtendedDataset, ClinicalDataset
from models.mapping import model_map
from utils import Losses, seed_everything, LinearWarmupCosineAnnealingLR, update_paths
from procedures import train_model, train_model_SSL, inference_comparative, train_model_comparative_SSL, train_model_comparative, inference_comparative_SSL
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-save-dir", default="$ROOT/checkpoints/comparative_PropIEDB_ImmunoIEDB_ImmunoCancer/", type=str)
    parser.add_argument("--figure-save-dir", default="$ROOT/figures/comparative_PropIEDB_ImmunoIEDB_ImmunoCancer/", type=str)
    ...
    # Weights and biases.
    wandb.init(
        project="ImmunoPred-Cancer-Paper", # set the wandb project where this run will be logged
        entity=config.wandb_username,
        name=f"Comparative-PropIEDB_ImmunoIEDB_ImmunoCancer:{model_str}",  # display name of the run
    ...
    dataset_ft = ImmunoPredDatasetComparative(config,
    ...
    train_dataset_pt1, val_dataset_pt1, test_dataset_pt1 = torch.utils.data.random_split(dataset_pt1, [0.7, 0.15, 0.15], generator)
    train_dataset_ft, val_dataset_ft, test_dataset_ft = torch.utils.data.random_split(dataset_ft, [0.7, 0.15, 0.15], generator)
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=config.learning_rate_pretrain, weight_decay=1e-4)
    ...
    train_split_dataset = SplitDataset(train_dataset_pt1, "train", binary=True, full=config.full_sequence, comparative=False, return_amino_acid=config.self_supervision)
    val_split_dataset = SplitDataset(val_dataset_pt1, "val", binary=True, full=config.full_sequence, comparative=False, return_amino_acid=config.self_supervision)
    test_split_dataset = SplitDataset(test_dataset_pt1, "test", binary=True, full=config.full_sequence, comparative=False, return_amino_acid=config.self_supervision)
    ...
    if config.self_supervision:
        ...
        train_losses, val_losses = train_model_SSL(config, device, model, train_loader, val_loader, optimizer, losses.BCE_loss_SSL)
    else:
        ...
        train_losses, val_losses = train_model(config, device, model, train_loader, val_loader, optimizer, losses.BCE_loss)
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=config.learning_rate_finetune, weight_decay=1e-4)
