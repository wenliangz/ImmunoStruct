...
from data_loading import ImmunoPredDatasetComparative, collate, SplitDataset, collate_amino_acid, ClinicalDataset, ExtendedDataset
from models.mapping import model_map
from utils import Losses, seed_everything, LinearWarmupCosineAnnealingLR, update_paths
from procedures import inference_comparative_SSL, inference_comparative, train_model_comparative_SSL, train_model_comparative
...
if __name__ == "__main__":
    ...
    wandb.init(
        ...
        name=f"Comparative-PropCancer_ImmunoCancer:{model_str}",  # display name of the run
    ...
    dataset_pt_ft = ImmunoPredDatasetComparative(config,
    ...
    train_split_dataset = SplitDataset(train_dataset_pt, "train", binary=False, full=config.full_sequence, comparative=True, return_amino_acid=config.self_supervision)
    val_split_dataset = SplitDataset(val_dataset_pt, "val", binary=False, full=config.full_sequence, comparative=True, return_amino_acid=config.self_supervision)
    test_split_dataset = SplitDataset(test_dataset_pt, "test", binary=False, full=config.full_sequence, comparative=True, return_amino_acid=config.self_supervision)
